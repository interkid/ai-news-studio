...
- M5進行中: キャラ刷新完了（うさぎペア・ピクサー調3DCG・11状態、`assets/characters/`にコミット済み）、
  ...
  演出バリエーション（冒頭drop_slam+感情連動エフェクト）実装済み・目視確認済み（2026-07-08）。
  ネタ選定は「ジャンル6種（ホットニュース×2/テック更新/効率化事例/論文/働き方/将来ビジョン）を
  1週間で1周する曜日ローテ+当日ジャンルTOP3選抜」へ変更し実装済み（2026-07-08、
  `shared/genres.py`。実CIでの動作確認は次回run時）。
  残り = LINEからの手動ストック → TOP3からの選択生成ボタン、
